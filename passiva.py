...
# Inicializado diretamente com o IP público da sua VM no Google Cloud
...
st.sidebar.markdown("---")
st.sidebar.subheader("🔗 Endpoints Ativos")
st.sidebar.caption("Use os botões de cópia rápida para configurar os seus emissores externos.")
...
with tab_rest:
    ...
    with col2:
        ...
        rest_url = st.text_input("URL da API REST de Destino", value=default_rest_url)
    ...
    if st.button("Disparar Requisição REST", type="primary", use_container_width=True):
        if not rest_url:
            ...
            try:
                ...
                with st.spinner("A enviar requisição REST..."):
...
# ==========================================
# ABA 2: PASSIVA - ENVIAR SOAP XML (WSDL TOLEDO)
# ==========================================
with tab_soap:
    ...
    if st.button("Carregar WSDL e Mapear Métodos", use_container_width=True):
        try:
            with st.spinner("A ler e analisar o documento WSDL da Toledo Brasil..."):
                settings = Settings(strict=False, xml_huge_tree=True)
                client = Client(wsdl=wsdl_url, settings=settings)
                st.session_state.soap_client = client
                st.success("✅ Estruturas e operações WSDL mapeadas com sucesso!")
        except Exception as e:
    ...
    if st.session_state.get("soap_client"):
        ...
        with col_op1:
            selected_operation = st.selectbox("Selecione a Operação (WebMethod)", operations)
        with col_op2:
            force_endpoint = st.text_input("Forçar Endpoint de Destino Real", value=inferred_endpoint)
        
        # Algoritmo de extração recursiva para montar o esqueleto XML
...
{xml_inner_body}
        ...
        st.markdown("**Envelope SOAP XML (Edite os valores das tags diretamente abaixo)**")
        soap_xml_payload = st.text_area("XML da Mensagem SOAP", value=xml_template, height=350)
        
        if st.button("Disparar Requisição SOAP XML", type="primary", use_container_width=True):
            try:
                with st.spinner(f"A disparar XML bruto para o método {selected_operation}..."):
                ...
                if response.status_code == 200:
                    ...
                    st.error(f"⚠️ Resposta com erro ou exceção do servidor. Status Code: {response.status_code}")
...
with tab_monitor:
    ...
    st.markdown("Esta aba consome a API da sua máquina virtual do GCP para listar o histórico em tempo real.")
    ...
    try:
        ...
        with st.spinner("A aceder ao servidor de logs remoto..."):
            ...
            if not logs:
                ...
                for idx, log in enumerate(logs):
                    with st.expander(f"[{log['data_hora']}] Requisição {log['tipo']} Intercetada"):
                        ...
                        st.json(log["headers"])
                        st.markdown(f"**Conteúdo Recebido (Payload/Body):**")
                        if log["tipo"] == "REST":
                            st.json(log["payload"])
                        else:
                            st.code(log["payload"], language="xml")
        else:
            st.error(f"Não foi possível obter os logs. Código de resposta da API: {response.status_code}")
            
    except requests.exceptions.ConnectionError:
        st.error(f"❌ Não foi possível estabelecer conexão com o servidor em `{api_base_url}`.\n\n"
                 f"**Validações necessárias:**\n"
                 f"1. Verifique se executou o comando `uvicorn ativa:app --host 0.0.0.0 --port 8000` dentro da sessão ativa da sua VM.\n"
                 f"2. Certifique-se de que a regra de Firewall `abrir-porta-8000-gcp` que criou na rede `default` para o tráfego `0.0.0.0/0` já terminou de propagar no GCP.")
    except Exception as e:
